...
const within = async <T>(promise: Promise<T>, label: string, timeoutMs = 5000): Promise<T> => {
...
describe('RabbitMQ integration', () => {
  it(
    'publishes, consumes, cancels and recovers an exclusive topology',
    {
      timeout: 20000,
    },
    async () => {
      ...
      try {
            ...
            queues: [
              {
                ...
                durable: true,
                autoDelete: false,
                bindTo: [{ exchange, routingKey: 'text' }],
              },
            ],
          }),
          stage,
            ...
            queues: [
              {
                ...
                exclusive: true,
                autoDelete: true,
                bindTo: [{ exchange, routingKey: 'created' }],
              },
            ],
          }),
          stage,
        ...
        const recoveringIterator = client
          .listen(exclusiveQueue, {
            routingKeys: ['created'],
          })
          [Symbol.asyncIterator]()
        ...
        await waitFor(async () => {
          ...
          return [...subscriptions].some(
            (subscription) =>
              subscription.queueName === exclusiveQueue && subscription.consumerTag !== undefined,
          )
        })

        stage = 'forced connection close'
        const originalConnection = (client as unknown as { connection: ChannelModel }).connection
        await within(originalConnection.close(), stage)

        stage = 'publish and delivery after reconnect'
        await within(client.publish(exchange, 'created', { id: 'after-reconnect' }), stage)
        ...
        if (!recovered.done) {
          ...
          assert.deepEqual(recovered.value.data, { id: 'after-reconnect' })
        ...
        throw new Error(`RabbitMQ integration failed during ${stage}`, { cause: error })
      } finally {
